...
int main(){
    int venta[SUCURSALES][MES]={{0}}, sucursal, mes, cantidad=0, cantidadVentas=0, objetivoMesual[MES]={0};
    char objetivo;
...
int Carga(int ventas[][MES], int sucursal, int mes, int cantidad, char objetivo, int objetivoMensual[]){
    ...
    printf("-Sucursal: ");
    sucursal=LeeYValida(999, 1010, 0);
    while(sucursal!=0){
        do{    
            printf("-Mes: ");
            mes=Ingreso(1, MES);
        ...
        }while(ventas[sucursal-1000][mes-1]!=0);
        printf("-Cantidad: ");
