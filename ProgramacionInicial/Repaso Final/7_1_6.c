...
int CuentaPorSector(int[], int, int);
void PagosTotales(int[], int[], int);
int Busqueda(int[], int, int);

int main(){
    int dni[EMPLEADOS], codigoSector[EMPLEADOS], sueldo[EMPLEADOS], sector[5]={0};
    int cantidadEmpleados, contador=0;

    cantidadEmpleados=Carga(dni, codigoSector, sueldo, EMPLEADOS);

    for(int i=0; i<5; i++){
        sector[i]=CuentaPorSector(codigoSector, cantidadEmpleados, i);
    }
    for(int i=0; i<5; i++){
...
int CuentaPorSector(int codigo[], int cantidad, int indice){
